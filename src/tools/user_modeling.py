...
# 数据库路径
USER_MODELING_DB_PATH = Path(os.path.expanduser("~")) / ".seed" / "memory" / "user_modeling.db"
...
class UserModelingLayer:
    ...
    def _extract_behaviors_from_tools(self, tool_calls: list[dict]) -> list[dict[str, Any]]:
        """从工具调用中提取行为模式"""
        behaviors = []
    ...
    async def _detect_conflicts(self, observations: list[dict[str, Any]]) -> list[dict[str, Any]]:
        """检测新证据与旧模型的矛盾
    ...
    async def _reason_about_conflicts(self, conflicts: list[dict[str, Any]]) -> dict[str, Any]:
        """内部推理讨论
